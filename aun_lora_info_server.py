...
def _extract_trained_words_from_metadata(metadata: dict[str, Any]) -> list[str]:
...
def _extract_civitai_payload(info: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "trained_words": _normalize_words(_pick_first(info, "trainedWords")),
...
def _merge_live_civitai_payload(payload: dict[str, Any], civitai_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not merged.get("trained_words"):
        merged["trained_words"] = civitai_payload.get("trained_words") or []
    ...
    if merged.get("trained_words"):
        _upsert_field(fields, "Trained Words", merged.get("trained_words"))
...
def _build_payload(lora_name: str, lora_path: str) -> dict[str, Any]:
    ...
    trained_words = civitai_payload.get("trained_words") or _extract_trained_words_from_metadata(safetensors_metadata)

    fields: list[dict[str, str]] = []

    def add_field(label: str, value: Any, href: str | None = None) -> None:
        text = _format_value(value, 800)
        if not text:
            return
        entry = {"label": label, "value": text}
    ...
    add_field("Name", title if title != stem else None)
    add_field("Base Model", base_model)
    add_field("Author", civitai_payload.get("creator"))
    add_field("Trained Words", trained_words)
    add_field("Strength Min", civitai_payload.get("strength_min"))
    add_field("Strength Max", civitai_payload.get("strength_max"))
    ...
    previews = _collect_local_previews(lora_path)
    if not previews:
        previews = civitai_payload.get("remote_previews") or []
