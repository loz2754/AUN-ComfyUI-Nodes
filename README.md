...
- Preset Manager (`AUNPresetManager`) scan a target node live (Setup button) and edit keyword-triggered preset rows (typed fields per widget, capture/copy/paste, JSON import/export, widget include/exclude chips and a field search filter). Outputs the selected row's values as a JSON string, the matched keyword, and the matched index, plus up to 12 manual value slots (ANY type) labelled with the widget names for wiring directly into a target node's converted inputs or into nodes such as AUNSaveImageV2 filename fields. Double-click toggles compact mode with a canvas-drawn match preview.
- Apply Preset To Node (`AUNApplyPresetToNode`) write a JSON object of widget values (e.g. Preset Manager's selected_values) into a target node's inputs, found by numeric ID or title. Values are applied at queue time — before validation and caching — so the target always executes with the preset values, even on cached re-runs. Linked inputs are never overwritten, values are coerced to the target's input types, the frontend widgets update live after the run, and the node's footer shows a scrollable confirmation summary (wheel + draggable scrollbar). An optional only_widgets filter (comma-separated names, wildcards and substrings supported) restricts which values apply, and an optional aliases input (source=target pairs, e.g. seed=seed_value) renames values before applying — so several Apply nodes can fan out from one Preset Manager to different targets with different widget names. Double-click toggles compact mode: config widgets hide, sockets converge to dots, and the footer takes over the node.
