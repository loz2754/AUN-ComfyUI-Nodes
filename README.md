...
- Scan And Show Widgets (`AUNScanAndShowWidgets`) scan any node by ID or title, display all its widget values as overlay cards inside the node, and provide up to 350 dynamic ANY-type output slots for wiring. Features a filter button (F) in the title bar with include/exclude wildcard patterns, collapse connections mode, show/hide data type badges, and configurable max value length.
