...
- Random Multi-LoRA Model Loader (Experimental) (`AUNRandomLoraModelOnlyMulti`) selects and applies multiple LoRAs based on prompt index, supporting up to 20 prompts with 3 LoRA slots each. Features compact mode with overlay UI for strength/trigger editing, drag-to-reorder support, and footer with combined trigger words display.
- LoRA Stack With Triggers Model Clip (`AUNLoraStackWithTriggersModelClip`) stacks multiple LoRAs with per-slot trigger words and separate model/clip strength control. Supports up to 10 slots with full compact mode featuring overlay UI, drag-to-reorder, and footer display. Successor to the deprecated AUNLoraStackWithTriggers with enhanced functionality.
