...
class AUNTextIndexSwitch5Diffusers:
    MAX_SLOTS = 20
    ...
    RETURN_NAMES = (
        "text", "label", "index",
        "diffusion_name", "clip_name", "vae_name", "clip_type",
    ...
    " Also scans the selected text for key=value tokens (diffusion_name, clip_name, vae_name, clip_type, sampler, scheduler, cfg, steps, seed) and outputs each as a typed value; those tokens are removed from the text output."
    )
    ...
    def text_index_switch(self, minimum, maximum, mode, index, slot_count, range, unique_id=None, extra_pnginfo=None, **kwargs):
        ...
        self._record_pginfo(
            ...
            unique_id,
            {
                "node": "AUNTextIndexSwitch5Diffusers",
        ...
        param_outputs = []
        for param_key in ("diffusion_name", "clip_name", "vae_name", "clip_type",
                          "sampler", "scheduler", "cfg", "steps", "seed"):
