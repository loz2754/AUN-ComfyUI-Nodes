class AUNMultiMuteIndex:
    ...
    def INPUT_TYPES(cls):
        ...
        inputs["Index"] = ("INT", {
            ...
            "tooltip": "Select the set index (1-20) to keep active. Only node IDs from this set remain unmuted; others are muted.",
        })
        for i in range(1, 21):
    ...
    OUTPUT_NODE = True
    DESCRIPTION = "Control mute state of multiple node sets using an index. Select an index to unmute one set while muting the others."
            ...
            for group_idx in range(slot_count):
                ...
                if node_ids_str:
                    try:
                        ...
                        print(f"[AUNMultiMuteIndex] Invalid node ID string for set {group_idx + 1}: '{node_ids_str}'. Must be comma-separated integers. Error: {e}")

            if node_states:
                bypass_clear = [{"node_id": nid, "is_active": True} for nid in node_states.keys()]
                PromptServer.instance.send_sync("AUN_node_bypass_state", {"updates": bypass_clear})
