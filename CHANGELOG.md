...
- All legacy frontend extensions migrated from `app.registerExtension` to `registerLegacyExtension`; settings reads/writes go through `aun-settings.js`.
- LoRA stack legacy extensions skip on Nodes 2.0 in favor of the Vue ports.
...
- Replaced AUNAutoPopulatePresets with the two-node preset architecture (Preset Manager + Apply Preset To Node).
- Collapse Connections SKIP_CLASSES updated to include the new preset nodes.

### Removed

- AUNAutoPopulatePresets and all related frontend code (setup dialog, widgets modal).
...
- AUNInputsBasic collapse mode now converges input slots together with outputs and shrinks the node height accordingly.
- AUNTextIndexSwitch3/4/5 compact mode: param output sockets converge to a single dot at the slot below the index output, and the node height shrinks to fit the visible content.
...
- Image slider comparer node height could not be decreased manually. `computeSize` enforced the saved user height as an absolute minimum via `Math.max`, preventing the user from dragging the node shorter.
...
- `_sanitize_for_json` to ensure JSON-safe output from any input type

### Changed

- Model/CLIP/VAE filenames now shown in Show Any Multi display instead of raw object names

### Fixed

- Manual node resize height no longer lost when connecting/disconnecting slots
