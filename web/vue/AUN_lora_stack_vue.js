/**
 * Vue frontend implementation of the LoRA stack with triggers.
 *
 * Replaces web/AUN_lora_stack_with_triggers.js while Nodes 2.0 is active.
 *
 * Patterns (see AGENTS.md "Vue frontend 1.53.x findings"):
 *  - compact mode removes widgets (the only reliable visual mechanism),
 *    stashes their values in node.properties and keeps the widget objects
 *    in node.__AUN_allWidgets — AUN_fix_prompt_missing_inputs.js re-injects
 *    the values into the prompt at queue time
 *  - compact rows are fixed-position DOM overlays driven by a continuous
 *    requestAnimationFrame loop (they follow node movement, hide when the
 *    node expands or leaves the screen) — works on both 1.49.x canvas-drawn
 *    2.0 and 1.53.x DOM-rendered frontends
 *  - the first row Y is measured at runtime (widget last_y on canvas-drawn
 *    frontends, DOM measurement of the apply_stack widget row on vueNodes)
 *    and shared by rows and node height so they never disagree
 *  - re-added widgets get their value re-asserted after addWidget and their
 *    callbacks re-wired (restored widgets are new objects)
 *  - vueTriggerWorkflowCapture() after mutations so the autosave snapshot
 *    includes them
 *  - node.widgets is never reassigned — reorder via in-place sort
 *  - widgets whose input carries a link are never removed
 */
...
const COMPACT_VALUES_PROP = "_AUN_lora_compactValues";
const MAX_SLOTS = 10;
...
// Registry maintenance: one entry per widget NAME. Replacing by name
// keeps the latest widget object (and its live value) after compact/expand
// cycles — duplicate stale entries were the source of value disconnects.
...
function formatCompactLoraLabel(widget, slotIndex) {
  const v = normalizeLoraWidgetValue(widget?.value);
  return v ? loraBasename(v) : `Slot ${slotIndex}`;
}
...
/**
 * Measure where the first compact row should sit, in node-local graph
 * coordinates. On DOM-rendered frontends the apply_stack widget row is
 * measured relative to the node element (no reliance on the legacy
 * canvas transform); on canvas-drawn frontends the widget's last_y is
 * used; otherwise the stored measurement or an estimate.
 */
...
.aun-lora-compact-row .aun-lora-row-strength{display:flex;align-items:center;gap:3px;cursor:ew-resize;user-select:none;touch-action:none;}
...
.aun-lora-compact-row .aun-lora-row-strength-value{min-width:30px;text-align:center;}
...
function getStrengthConfig(node, slotIndex) {
  // The strength widgets come from a fixed def we control
  // (AUNLoraStackWithTriggers.py): FLOAT, min -20, max 20, step 0.01.
  // Some frontends report a different step on the widget options — trust
  // the def instead so overlay and base node always agree.
  return { min: -20, max: 20, step: 0.01 };
}

function syncCompactRow(node, slotIndex) {
  const row = node._AUN_compactRows?.[slotIndex - 1];
  if (!row) return;
  const loraW = getWidget(node, `lora_${slotIndex}`);
  const strengthW = getWidget(node, `strength_model_${slotIndex}`);
  const enabledW = getWidget(node, `enabled_${slotIndex}`);
  const checkbox = row.querySelector(".aun-lora-row-checkbox");
  const label = row.querySelector(".aun-lora-row-label-text");
  const strength = row.querySelector(".aun-lora-row-strength-value");
  const v = normalizeLoraWidgetValue(loraW?.value);
  if (checkbox) checkbox.checked = !!normalizeScalar(enabledW?.value);
  if (label) {
    label.textContent = v ? loraBasename(v) : `Slot ${slotIndex}`;
    label.classList.toggle("aun-lora-row-label-none", !v);
  }
  if (strength) {
    const s = Number(normalizeScalar(strengthW?.value));
    strength.textContent = Number.isFinite(s) ? s.toFixed(2) : "0.00";
  }
}
...
function buildCompactRow(node, slotIndex) {
  ...
  checkbox.addEventListener("change", () => {
    ...
    syncCompactRow(node, slotIndex);
    captureAunWidgetValues(node);
    forceFullRedraw(node);
    vueTriggerWorkflowCapture();
  });

  // Drag grip for slot reordering.
  ...
  makeLoraLabelClickable(node, `lora_${slotIndex}`, labelWrap, labelText, {
    formatLabel: (widget) => formatCompactLoraLabel(widget, slotIndex),
    onChanged: () => {
      syncCompactRow(node, slotIndex);
      captureAunWidgetValues(node);
      forceFullRedraw(node);
  ...
  strengthWrap.className = "aun-lora-row-strength";
  const minusBtn = document.createElement("button");
  minusBtn.textContent = "\u2212";
  const strengthText = document.createElement("span");
  strengthText.className = "aun-lora-row-strength-value";
  const plusBtn = document.createElement("button");
  plusBtn.textContent = "+";
  const strengthW = getWidget(node, `strength_model_${slotIndex}`);
  const applyStrength = (val) => {
    const { min, max, step } = getStrengthConfig(node, slotIndex);
    const clamped = clampNumber(
      roundToStep(val, step),
      min,
      max,
    );
    if (strengthW) strengthW.value = clamped;
    syncCompactRow(node, slotIndex);
    captureAunWidgetValues(node);
    forceFullRedraw(node);
    vueTriggerWorkflowCapture();
  ...
  minusBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    const { step } = getStrengthConfig(node, slotIndex);
    applyStrength((Number(normalizeScalar(strengthW?.value)) || 0) - step);
  });
  ...
  strengthWrap.addEventListener("pointerdown", (e) => {
    if (e.button !== 0) return;
    strengthDrag = {
      node,
      slotIndex,
      startX: e.clientX,
      startVal: Number(normalizeScalar(strengthW?.value)) || 0,
      active: false,
    };
    e.preventDefault();
  });
  strengthWrap.appendChild(minusBtn);
  strengthWrap.appendChild(strengthText);
  strengthWrap.appendChild(plusBtn);
  ...
  row.append(checkbox, grip, labelWrap, strengthWrap, infoBtn);
...
function positionCompactRowsCore(node, fast) {
  ...
  for (const row of rows) {
    ...
    if (!Number.isFinite(slotIndex) || slotIndex > numSlots) {
...
function positionCompactRowsFromCanvas(node, fast) {
  ...
  if (globalThis.__AUN_LORA_DEBUG) {
    try {
      const snap = JSON.stringify({ compact, collapsed, occluded, onScreen, resizing });
        ...
        console.info(
          "[AUN] lora hide: " +
            JSON.stringify({
              id: node.id,
              fast: !!fast,
              hidden: !compact || collapsed || occluded || !onScreen || resizing,
              compact,
              collapsed,
              occluded,
              onScreen,
  ...
  if (!compact || collapsed || occluded || !onScreen || resizing) {
    for (const row of rows) row.style.display = "none";
    return;
  }
  positionCompactRowsCore(node, fast);
}
...
function removeForCompact(node, name) {
  ...
  const stash =
    ...
    (node.properties[COMPACT_VALUES_PROP] = {});
  // On a workflow load the stash is authoritative (widgets were re-created
  // with defaults) — restore before removal so __AUN_allWidgets keeps the
  // saved values for prompt injection.
  if (stash[name] !== undefined && w.value !== stash[name]) {
    w.value = stash[name];
  }
  registerAllWidget(node, w);
  node._AUN_lora_removed.set(name, w);
  stash[name] = w.value;
  try {
    node.removeWidget(w);
  } catch (_) {}
...
function restoreAllRemoved(node) {
  const removed = node._AUN_lora_removed ?? new Map();
  ...
  for (const [name, stale] of [...removed.entries()]) {
    ...
    // Prefer the removed widget's live value (row edits update it) over the
    // compact-time stash.
    const value =
  ...
  node._AUN_lora_removed = new Map();
  node.properties[COMPACT_VALUES_PROP] = {};
  reorderWidgets(node);
  // Restored widgets are NEW objects — their callbacks must be re-wired
  // (num_slots changes, LoRA changes etc.).
...
function applyCompact(node) {
  ...
  if (compact) {
    for (const w of [...(node.widgets || [])]) {
      ...
      if (hasLinkedInput(node, w.name)) continue; // never remove linked widgets
  ...
  try {
    if (globalThis.__AUN_LORA_DEBUG) {
      console.info(
        "[AUN] lora applyCompact: " +
          JSON.stringify({
            compact,
            numSlots: getNumSlots(node),
            numSlotsRaw: normalizeScalar(getWidget(node, "num_slots")?.value),
            widgets: (node.widgets || []).map((w) => w.name),
            stash: {
              num_slots: node.properties[COMPACT_VALUES_PROP]?.num_slots,
              lora_1: node.properties[COMPACT_VALUES_PROP]?.lora_1,
              enabled_1: node.properties[COMPACT_VALUES_PROP]?.enabled_1,
              strength_model_1:
                node.properties[COMPACT_VALUES_PROP]?.strength_model_1,
            },
            registry: (node.__AUN_allWidgets || [])
              .filter((w) => w?.name === "num_slots")
              .map((w) => normalizeScalar(w.value)),
          }),
      );
    }
  } catch (_) {}
...
function setupCollapseConnections(node) {
  if (!node || node._AUN_ccPatched) return;
  node._AUN_ccPatched = true;
  const collapsed = () => !!node.properties?._AUN_collapseConnections;
  if (typeof node.getOutputPos === "function") {
    const orig = node.getOutputPos;
    node.getOutputPos = function (slot, out) {
      if (collapsed()) {
        return [this.pos[0] + this.size[0], this.pos[1] + 8];
      }
      return orig.apply(this, arguments);
    };
  }
  if (typeof node.getInputPos === "function") {
    const orig = node.getInputPos;
    node.getInputPos = function (slot, inObj) {
      if (collapsed()) {
        return [this.pos[0] + this.size[0], this.pos[1] + 8];
      }
      return orig.apply(this, arguments);
...
function wireWidgetCallbacks(node) {
  ...
  if (numW && !numW._AUN_loraWired) {
    numW._AUN_loraWired = true;
  ...
  for (let i = 1; i <= MAX_SLOTS; i++) {
    ...
    if (!lw || lw._AUN_loraWired) continue;
    lw._AUN_loraWired = true;
    ...
    lw.callback = function (v) {
      ...
      try {
        ...
        if (triggerW && !String(triggerW.value ?? "").trim()) {
          triggerW.value = newLora ? loraBasename(newLora) : "";
        }
        if (prevLora && triggerW) {
          const cur = String(triggerW.value ?? "").trim();
...
function startLiveMonitor(node) {
  if (node._AUN_loraMonitor) return;
  node._AUN_loraMonitor = setInterval(() => {
    try {
      if (!isTargetNode(node)) return;
...
function ensureWidgetSerialization(node) {
  if (typeof node.serialize !== "function" || node._AUN_loraSerializeSetup) {
    return;
  }
  node._AUN_loraSerializeSetup = true;
...
function setupNode(node) {
  ...
  applyCompact(node);
  scheduleCompactHeightRefresh(node);
  if (!node.__AUN_loraRemovedPatched) {
    node.__AUN_loraRemovedPatched = true;
    const origRemoved = node.onRemoved;
    node.onRemoved = function (...args) {
      try {
        disposeCompactRows(node);
      } catch (_) {}
      if (node._AUN_loraMonitor) {
        clearInterval(node._AUN_loraMonitor);
        node._AUN_loraMonitor = null;
      }
      if (node.__AUN_compactHeightTimer) {
        clearTimeout(node.__AUN_compactHeightTimer);
        node.__AUN_compactHeightTimer = null;
...
registerVueExtension({
  name: "AUN.LoraStackWithTriggers.Vue",

  async setup() {
    vueRegisterNodeDblClick((node) => {
      if (node.comfyClass === NODE_TYPE) toggleCompactMode(node);
    });
    // Keep rows positioned while the canvas moves/zooms (belt-and-braces
    // on top of the continuous RAF loop).
    const cv = app?.canvas?.canvas;
    ...
    if (typeof window !== "undefined") {
      ...
      // rows (zero layout reads) so they track the node 1:1 while dragging.
    ...
    if (typeof window !== "undefined") {
      window.addEventListener("resize", () => scheduleCompactRowsUpdate());
      // Global drag handlers for row strength/reorder (module-level state).
      window.addEventListener("pointermove", (e) => {
        if (strengthDrag) {
          const dx = e.clientX - strengthDrag.startX;
          if (!strengthDrag.active && Math.abs(dx) < 5) return;
          strengthDrag.active = true;
          const strengthW = getWidget(
            strengthDrag.node,
            `strength_model_${strengthDrag.slotIndex}`,
          );
          const { min, max, step } = getStrengthConfig(
            strengthDrag.node,
            strengthDrag.slotIndex,
          );
          const val = clampNumber(
            roundToStep(strengthDrag.startVal + dx * step * 0.4, step),
            min,
            max,
          );
          if (strengthW) strengthW.value = val;
          syncCompactRow(strengthDrag.node, strengthDrag.slotIndex);
        } else if (reorderDrag && e.buttons === 1) {
          if (
          ...
          if (reorderDrag.moved) {
            // Highlight the row under the pointer as the drop target.
            const t = document
              .elementFromPoint(e.clientX, e.clientY)
      ...
      window.addEventListener("pointerup", (e) => {
        if (strengthDrag) {
          const wasActive = strengthDrag.active;
          const node = strengthDrag.node;
          strengthDrag = null;
          if (wasActive && node) {
            captureAunWidgetValues(node);
            forceFullRedraw(node);
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    // through node.onDblClick — patch the prototype like the compat
    // extension does. Harmless on DOM-rendered frontends.
    if (!nodeType.prototype._AUN_lora_vueDblClickPatched) {
      nodeType.prototype._AUN_lora_vueDblClickPatched = true;
  ...
  getNodeMenuItems(node) {
    ...
    return [
      ...
      {
        content: collapsed
          ? "AUN: Expand connections"
          : "AUN: Collapse connections",
        callback: () => {
          node.properties = node.properties || {};
          node.properties._AUN_collapseConnections = !collapsed;
          forceFullRedraw(node);
          setTimeout(() => vueTriggerWorkflowCapture(), 50);
        },
      },
