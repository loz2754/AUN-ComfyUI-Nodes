...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      if (this.properties._AUN_compactMode === undefined)
        this.properties._AUN_compactMode = false;

      // Add a right-click menu to toggle compact mode
      const origGetMenuOptions = this.getMenuOptions;
      this.getMenuOptions = function () {
        const options = origGetMenuOptions
          ? origGetMenuOptions.apply(this, arguments)
          : [];
        options.push(null);
        options.push({
      ...
      const updateCompactMode = () => {
        ...
        if (compact) {
          ...
          this._AUN_origComputeSize =
            this._AUN_origComputeSize || this.computeSize;
