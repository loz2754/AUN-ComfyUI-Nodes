...
app.registerExtension({
	...
	async setup() {
		// Listen for the custom message from the Python backend to set the title
		api.addEventListener("AUN.set_node_title", ({ detail }) => {
			if (!detail.node_id || !app.graph) {
				return;
			}
			
			// Find the node on the canvas by its ID
			const node = app.graph.getNodeById(detail.node_id);
			if (node) {
				// Set the title and mark the canvas as dirty to trigger a redraw
				node.title = detail.title || `AUN Show Text With Title`;
				app.graph.setDirtyCanvas(true, true);
			}
		});
	},
    // This function is called before a node is registered with the graph
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "AUNShowTextWithTitle") {
            function populate(text) {
                ...
                for (let list of v) {
			        ...
			        for (const l of list) {
                        const w = ComfyWidgets["STRING"](this, "text_" + 
                        this.widgets?.length ?? 0, ["STRING", { multiline: true }], app).widget;  
        ...
        nodeType.prototype.onConfigure = function () {
            onConfigure?.apply(this, arguments);
                ...
                if (widgets_values?.length) {
                    ...
                    requestAnimationFrame(() => {
                        populate.call(this, widgets_values.slice
