...
const VALID_TOGGLE_RESTRICTIONS = [
  "default",
  "max one",
  "always one",
  "iterate",
...
const getAllGraphs = (root) => {
  ...
  const normalizeGraph = (candidate) => {
    ...
    if (candidate.graph && (candidate.graph._nodes || candidate.graph.nodes))
  ...
  const visit = (graphLike) => {
    ...
    if (nodes) {
      ...
      if (Array.isArray(nodes)) nodes.forEach(visitNode);
        ...
        Object.values(nodes).forEach((value) => visitNode(value));
    }
  };
...
const resolveSubgraphByKey = (key) => {
  ...
  try {
    ...
    if (collection instanceof Map) {
      ...
      if (value.graph && (value.graph._nodes || value.graph.nodes))
        return value.graph;
...
const getInnerGraphsFromNode = (node) => {
  ...
  const push = (value) => {
    ...
    const normalized =
      value.graph && (value.graph._nodes || value.graph.nodes)
        ? value.graph
        : value;
    if (normalized && (normalized._nodes || normalized.nodes))
...
const forEachNodeAndInnerNodes = (
  node,
  visitor,
...
const computeGroupSignature = (map) =>
  Array.from(map.values())
    .map((entry) => `${entry.title}:${entry.entries.length}`)
    .sort()
    .join("|");
...
const ensureGlobalGroupWatcher = () => {
  ...
  if (
    graph &&
    !graph.__AUN_groupChangeHooked &&
    typeof graph.change === "function"
  ) {
  ...
  if (bounds && bounds.every((value) => value === 0)) {
    ...
    if (ctx && node.updateArea) {
      try {
        ...
        console.warn("[AUN] updateArea failed", error);
      }
...
const applyUniversalUpdate = (mode, groupsPayload) => {
  ...
  groupsPayload.forEach(({ type, targets, is_active }) => {
    ...
    if (type === "Group") {
      ...
      const excludeSet = new Set(
        excludes.map((value) => String(value).trim()).filter(Boolean),
      );
      includes.forEach((target) => {
...
const ensureAllGroupsStateWidget = (node) => {
  ...
  const stored =
    typeof node.properties?.[STATE_WIDGET_NAME] === "string"
      ? node.properties[STATE_WIDGET_NAME]
      : "";
  widget.value = stored;
...
const updateOutputs = (
  node,
  { showOutputs, slotCount, useAllGroups, isGroupNode },
) => {
  ...
  while (node.outputs.length < target)
    node.addOutput(`Switch ${node.outputs.length}`, "BOOLEAN");

  if (!showOutputs) return;

  for (let i = 1; i < node.outputs.length; i++) {
    const rawLabel = isGroupNode
      ? getWidget(node, `group_name_${i}`)?.value
      : getWidget(node, `label_${i}`)?.value;
    const labelText =
...
const buildAllGroupsUI = (node, showGroupsUI, mode) => {
  ...
  const titles = Array.from(groups.values())
    .map((entry) => entry.title)
    .filter(Boolean)
    .sort((a, b) => a.localeCompare(b));
  ...
  const header = node.addWidget(
    "toggle",
    "All Groups",
    titles.length > 0 && active.size === titles.length,
    (value) => {
  ...
  titles.forEach((title) => {
    const widget = node.addWidget(
      ...
      (value) => {
        if (node._AUN_updatingGroups) return;
        node._AUN_updatingGroups = true;
        if (value) active.add(title);
        else active.delete(title);
...
const refreshGroupDropdowns = function refreshGroupDropdowns(
  force = false,
  sharedData,
) {
  if (
    (!this.widgets && !this.__AUN_allWidgets) ||
    !this.__AUN_isGroupNode ||
    this.__AUN_useAllGroups
  )
    return;
  ...
  for (let slot = 1; slot <= MAX_SLOTS; slot++) {
    ...
    const currentRaw =
      typeof original.value === "string" ? original.value.trim() : "";
    if (currentRaw) {
      const exactExists = titles.includes(currentRaw);
      if (!exactExists) {
        const lower = currentRaw.toLowerCase();
        const remapped = titles.find(
          (title) => title && title.toLowerCase() === lower,
        );
        if (remapped) {
    ...
    if (!combo) {
      combo = this.addWidget(
        "combo",
        `__AUN_group_combo_${slot}`,
        original.value || "",
        (value) => {
...
const refreshWidgets = function refreshWidgets() {
  ...
  const useAllGroups =
    isGroupNode && !!getWidget(this, "use_all_groups")?.value;
  ...
  for (let slot = 1; slot <= MAX_SLOTS; slot++) {
    ...
    const labelValue =
      typeof labelWidget?.value === "string" ? labelWidget.value.trim() : "";
    ...
    const showSlotDetails =
      withinRange &&
      (showFullInputs ||
        (showSelectedSlotsCompact &&
          allowCompactDetails &&
          (slotActive ||
            restriction === "max one" ||
            restriction === "always one")));
    const showGroupSlot =
      withinRange && isGroupNode && !useAllGroups && showFullInputs;
    if (switchWidget) {
      ...
      const hideForCompactSelection =
        showSelectedSlotsCompact && !slotHasTargets && !isNodeController;
      applyWidgetHiddenState(
        switchWidget,
        !withinRange || hideForUseAll || hideForCompactSelection,
      );
...
const syncTogglesWithGraph = function syncTogglesWithGraph() {
  ...
  let dirty = false;
  // Set flag to prevent executeInstant from being called when syncing switches
  ...
  if (this.__AUN_isGroupNode) {
    ...
    if (useAllGroups) {
      ...
      titles.forEach((title) => {
        ...
        const { total, disabled } = evaluateGroupState(
          entry?.entries || [],
          mode,
        );
        if (!total) return;
        const shouldBeActive = disabled === 0;
        if (shouldBeActive && !active.has(title)) {
          active.add(title);
          dirty = true;
        }
        if (!shouldBeActive && active.has(title)) {
        ...
        const widget = getAllTrackedWidgets(this).find(
          (w) => w._groupTitle === title,
        );
        if (widget && widget.value !== shouldBeActive) {
          widget.value = shouldBeActive;
          dirty = true;
        }
      });
      const header = getAllTrackedWidgets(this).find(
        (widget) => widget.name === "All Groups",
      );
      if (header) {
        const shouldAll = titles.length > 0 && active.size === titles.length;
        if (header.value !== shouldAll) {
          header.value = shouldAll;
          dirty = true;
        }
      }
...
const executeInstant = function executeInstant() {
  ...
  if (this.__AUN_isGroupNode) {
    if (this.__AUN_useAllGroups) {
      ...
      const titles = Array.from(map.values())
        .map((entry) => entry.title)
        .filter(Boolean)
        .sort((a, b) => a.localeCompare(b));
      ...
      if (activeTitles.length)
        groupsPayload.push({
          type: "Group",
          targets: activeTitles,
          is_active: true,
        });
      if (inactiveTitles.length)
        groupsPayload.push({
          type: "Group",
          targets: inactiveTitles,
          is_active: false,
        });
    } else {
      ...
      for (let slot = 1; slot <= slotCount; slot++) {
        ...
        const switchWidget = getWidget(this, `switch_${slot}`);
        const isActive =
          (switchWidget ? !!switchWidget.value : false) || allSwitch;
        names.forEach((name) => {
      ...
      const inactiveTitles = Array.from(inactiveGroups).filter(
        (name) => !activeGroups.has(name),
      );
      if (activeTitles.length)
        groupsPayload.push({
          type: "Group",
    ...
    const filteredInactiveIds = Array.from(inactiveIds).filter(
      (id) => !activeIds.has(id),
    );
    const filteredActiveTitles = Array.from(activeTitles);
    const filteredInactiveTitles = Array.from(inactiveTitles).filter(
      (title) => !activeTitles.has(title),
    );

    if (filteredActiveIds.length)
      groupsPayload.push({
        type: "ID",
        targets: filteredActiveIds,
        is_active: true,
      });
    if (filteredInactiveIds.length)
      groupsPayload.push({
        type: "ID",
        targets: filteredInactiveIds,
        is_active: false,
      });
    if (filteredActiveTitles.length)
      groupsPayload.push({
        type: "Title",
        targets: filteredActiveTitles,
        is_active: true,
      });
    if (filteredInactiveTitles.length)
      groupsPayload.push({
        type: "Title",
        targets: filteredInactiveTitles,
        is_active: false,
      });
  }
...
const enforceRestriction = (node, slot, value) => {
  ...
  if (restriction === "max one" || restriction === "always one") {
    if (value) {
      ...
      const anyOn = Array.from({ length: slotCount }).some(
        (_, index) => getWidget(node, `switch_${index + 1}`)?.value,
      );
      if (!anyOn) {
...
const attachSwitchHandlers = (node) => {
  for (let slot = 1; slot <= MAX_SLOTS; slot++) {
    ...
    widget.callback = function callback(value) {
      ...
      // Skip executeInstant if batch toggling or syncing toggles with graph
  ...
  node.__AUN_updateAutoHeight = () => {
    ...
    if (typeof node.computeSize === "function") {
      ...
      if (Array.isArray(originalWidgets) && originalWidgets.length) {
        const visibleWidgets = originalWidgets.filter(
