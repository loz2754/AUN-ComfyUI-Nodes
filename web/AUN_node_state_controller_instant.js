...
function applyCollapseToIds(ids, collapse) {
  for (const nodeId of ids) {
    ...
    if (!target) continue;
    // Directly set the collapsed flag without calling collapse() to avoid side effects
    if (!target.flags) target.flags = {};
    target.flags.collapsed = !!collapse;
  }
}
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, appInstance) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      const targetModeWidget = this.widgets.find(
        (w) => w.name === "target_mode",
      );
      const nodeIdsWidget = this.widgets.find((w) => w.name === "node_ids");
      let groupTitleWidget = this.widgets.find((w) => w.name === "group_title");
      const groupExcludeWidget = this.widgets.find(
        (w) => w.name === "group_exclude_titles",
      );
      const nodeTitlesWidget = this.widgets.find(
        (w) => w.name === "node_titles",
      );
      ...
      const forceFullRedraw = () => {
        try {
          ...
          if (
            this.size &&
            typeof this.size.length === "number" &&
            this.size.length >= 2
          ) {
      ...
      const ensureGroupTitleCombo = () => {
        try {
          ...
          if (groupTitleWidget.type === "combo") {
            ...
            this.addWidget(
              ...
              (v) => {
                ...
                if (originalCallback)
                  originalCallback.call(groupTitleWidget, v);
                forceFullRedraw();
              },
              { values: titles },
            );
          ...
          console.warn(
            "[AUNNodeStateController] ensureGroupTitleCombo error:",
            e,
          );
        }
      };
      ...
      const cacheNodeBounds = () => {
        ...
        for (const n of app.graph._nodes)
          boundsMap[String(n.id)] = ensureBounds(n);
        return boundsMap;
      };
      ...
      const refreshVisibility = () => {
        ...
        if (groupExcludeWidget)
          groupExcludeWidget.hidden = mode !== "Group Title";
        if (nodeTitlesWidget) nodeTitlesWidget.hidden = mode !== "Node Titles";
        if (nodeTitlePresetWidget)
          nodeTitlePresetWidget.hidden = mode !== "Node Titles";
        ...
        try {
          if (nodeTitlesWidget && !nodeTitlesWidget.hidden) {
            ...
            if (typeof nodeTitlesWidget.computeSize === "function") {
              ...
              const sz = nodeTitlesWidget.computeSize(
                nodeTitlesWidget.width || 200,
              );
              if (sz && sz[1] < 70) {
              ...
              nodeTitlesWidget.height = Math.max(
                nodeTitlesWidget.height || 0,
                70,
              );
            }
          }
          if (groupExcludeWidget && !groupExcludeWidget.hidden) {
            ...
            if (typeof groupExcludeWidget.computeSize === "function") {
              const gsz = groupExcludeWidget.computeSize(
                groupExcludeWidget.width || 200,
              );
              if (gsz && gsz[1] < 60) groupExcludeWidget.height = 60;
            } else {
              groupExcludeWidget.height = Math.max(
                groupExcludeWidget.height || 0,
                60,
              );
            }
          }
      ...
      const getAllNodeTitles = () =>
        Array.from(
          new Set(
            (app.graph._nodes || []).map((n) => n.title).filter((t) => t),
          ),
        ).sort();
      ...
      const ensureTitlePresetCombo = () => {
        try {
          ...
          if (targetModeWidget?.value !== "Node Titles") {
            if (nodeTitlePresetWidget) {
              nodeTitlePresetWidget.hidden = true;
            }
            return;
          }
          ...
          if (!nodeTitlePresetWidget) {
            ...
            nodeTitlePresetWidget = this.addWidget(
              "combo",
              "add_title",
              "",
              cb,
            ...
            nodeTitlePresetWidget.options.placeholder = titles.length
              ? "Add Title..."
              : "(No Titled Nodes Yet)";

            if (
              nodeTitlePresetWidget.value &&
              !titles.includes(nodeTitlePresetWidget.value)
            ) {
              nodeTitlePresetWidget.value = "";
            }
          ...
          console.warn(
            "[AUNNodeStateController] ensureTitlePresetCombo error",
            e,
          );
        }
      };
      ...
      if (!window.__AUNTitlePickerInit) {
        ...
        window.__AUNOpenTitlePicker = (
          ...
          try {
            const existing = document.getElementById(
              "AUN-title-picker-overlay",
            );
            if (existing) existing.remove();
          } catch {}
          const overlay = document.createElement("div");
          overlay.id = "AUN-title-picker-overlay";

          // Calculate position relative to the node
          let overlayLeft = 100,
            overlayTop = 100; // Default fallback position
          try {
          ...
          const buildList = () => {
            ...
            const current = (nodeTitlesWidgetRef.value || "")
              .split("\n")
              .map((s) => s.trim())
              .filter(Boolean);
            titles
              ...
              .forEach((t) => {
                ...
                if (added) {
                  ...
                  div.style.background = "#284";
                }
                div.onclick = () => {
                  const existingVal = nodeTitlesWidgetRef.value || "";
                  const lines = existingVal
                    .split("\n")
                    .map((l) => l.trim())
                    .filter(Boolean);
                  const idx = lines.indexOf(t);
                  if (idx === -1) lines.push(t);
                  else lines.splice(idx, 1); // toggle
        ...
        window.addEventListener("keydown", (e) => {
          if (e.key === "Escape") {
            const ov = document.getElementById("AUN-title-picker-overlay");
            if (ov) ov.remove();
          }
        });
      }
      // Ensure fallback button present
      let openPickerBtn = this.widgets.find(
        (w) =>
          w.__AUNInternal === "openTitlePicker" ||
          w.name === "openTitlePicker" ||
          w.name === "Title Picker",
      );
      if (!openPickerBtn) {
        // Create button with both label and button text as 'Title Picker'.
        openPickerBtn = this.addWidget(
          "button",
          "Title Picker",
          "Title Picker",
          () => {
          ...
          { tooltip: "Open node title picker overlay", serialize: false },
        );
        ...
        if (
          openPickerBtn.name === "" ||
          openPickerBtn.name === "openTitlePicker"
        )
          openPickerBtn.name = "Title Picker";
        if (openPickerBtn.value !== "Title Picker")
          openPickerBtn.value = "Title Picker";
      }
      ...
      if (targetModeWidget) {
        const prev = targetModeWidget.callback;
        targetModeWidget.callback = (v) => {
          prev?.call(targetModeWidget, v);
          forceRefreshVisibility();
          if (v === "Group Title") {
            ensureGroupTitleCombo();
          }
      ...
      const executeCombined = (val) => {
        ...
        if (mode === "Node IDs") {
          ...
          if (useMute) applyMuteToIds(ids, combined);
          else applyBypassToIds(ids, combined);
          ...
          if (groupExcludeWidget && groupExcludeWidget.value) {
            const exc = groupExcludeWidget.value
              .split(/[,\n;]+/)
          ...
          const useMute = !!useMuteWidget?.value;
          if (useMute) applyMuteToNodeArray(nodes, combined);
          else applyBypassToNodeArray(nodes, combined);
        } else if (mode === "Node Titles") {
          const titles = (nodeTitlesWidget?.value || "")
            .split(/[,\n;]+/)
            .map((l) => l.trim())
            .filter(Boolean);
          if (!titles.length) return;
      ...
      const executeActive = (val) => {
        ...
        if (mode === "Node IDs") {
          ...
          if (title) {
            ...
            const useMute = !!useMuteWidget?.value;
            if (useMute) applyMuteToNodeArray(nodes, !active);
            else applyBypassToNodeArray(nodes, !active);
          }
        } else if (mode === "Node Titles") {
          const titles = (nodeTitlesWidget?.value || "")
            .split(/[,\n;]+/)
