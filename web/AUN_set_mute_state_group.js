...
const NODE_TYPE = "AUNSetMuteStateGroup";

// Register with shared group watcher so a single interval serves both bypass & mute nodes
registerGroupNodeType(NODE_TYPE);

app.registerExtension({
  name: "AUN.SetMuteStateGroup.Event",
  async setup(app) {
    startGroupWatcher();

    // Apply mute state for one or multiple selected groups when server event is received
    api.addEventListener("AUN_set_mute_state_group", (event) => {
      try {
        ...
        for (const graph of allGraphs) {
          applyMuteToGroupsInGraph(graph, titles, isActive);
        }
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      this.setGroupMuteState = (groupTitles, isActive) => {
      ...
      this._setupMultiSelect = () => {
        ...
        const allToggle = this.addWidget("toggle", "All Groups", false, (v) => {
          ...
          this.setGroupMuteState(titles, v);
        ...
        for (const title of titles) {
          ...
          const tg = this.addWidget("toggle", `• ${title}`, initial, (v) => {
            ...
            w.value = Array.from(selectedSet).join(", ");
            this.setGroupMuteState([title], v);
            this._isCurrentlySettingState = false;
          }, { on: "🟢", off: "🔴" });
      ...
      this.syncTogglesWithGraph = () => {
        ...
        let allGroupsAreMuted = allGroups.length > 0;
        ...
        for (const [title, groups] of groupsByTitle.entries()) {
          ...
          for (const group of groups) {
            ...
            if (nodesInGroup.length) {
              if (!nodesInGroup.every((n) => n.mode === NODE_MODE.MUTED))
                titleIsMuted = false;
        ...
        if (allToggle) {
          ...
          else if (allGroupsAreMuted) newState = false;
          else newState = false;
...
function applyMuteToGroupsInGraph(graph, titles, isActive) {
  ...
  for (const node of graph._nodes || []) {
    if (isNodeInsideGroups(node, selectedGroups, boundsMap)) {
      node.mode = isActive ? NODE_MODE.ACTIVE : NODE_MODE.MUTED;
    }
  }
}
