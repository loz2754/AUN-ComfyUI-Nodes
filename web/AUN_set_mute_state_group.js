    ...
    try {
        ...
        for (const g of allGraphs) {
            ...
            for (const node of g._nodes) {
                if (node?.type === "AUNSetMuteStateGroup") {
...
app.registerExtension({
        ...
        api.addEventListener("AUN_set_mute_state_group", (event) => {
            try {
                ...
                for (const graph of allGraphs) {
                    ...
                    for (const node of (graph._nodes || [])) {
                        if (insideGroup(node, selectedGroups)) node.mode = isActive ? 0 : 2;
                    }
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "AUNSetMuteStateGroup") {
                ...
                this.setGroupMuteState = (groupTitles, isActive) => {
                    ...
                    for (const graph of allGraphs) {
                        ...
                        for (const node of (graph._nodes || [])) {
                            ...
                            if (inAny) node.mode = isActive ? 0 : 2;
                        }
                        if (graph.setDirtyCanvas) graph.setDirtyCanvas(true, true);
                    }
                    app.graph.setDirtyCanvas(true, true);
                    ...
                    for (const [title, groups] of groupsByTitle.entries()) {
                        ...
                        for (const group of groups) {
                            ...
                            if (nodesInGroup.length) {
                                hasNodes = true;
                                if (!nodesInGroup.every(n => n.mode === 2)) titleIsMuted = false;
