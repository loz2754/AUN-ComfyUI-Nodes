...
from .AUNSaveImage_helpers import (
    _sanitize_subfolder_path,
    _parse_lora_tag_text,
    extract_loras,
    get_timestamp,
    build_sidecar_timestamp,
...
class AUNSaveImage:
    ...
    DESCRIPTION = "Legacy image saver for workflows that provide separate path and filename inputs. Supports advanced filename customization, metadata embedding, and LoRA tracking via selected_lora input. Use Save Image V2 for new single path_filename workflows. Right-click -> \"Preview Mode\" or double-click to hide all widgets and focus on the image preview."
    ...
    def _prepare_metadata_comment(self, **kwargs):
        ...
        short_sampler = get_sampler_short_name(kwargs.get('sampler_name', ''))
        short_scheduler = get_scheduler_short_name(kwargs.get('scheduler', ''))
    ...
    def save_files(self, images, filename, path, extension, **kwargs):
        try:
            ...
            replacements = {
                ...
                "sampler_name": get_sampler_short_name(kwargs.get("sampler_name", "")),
                "scheduler": get_scheduler_short_name(kwargs.get("scheduler", "")),
